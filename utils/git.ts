...
export async function cloneRepository(
	...
	try {
		...
		await fs
			.rm(path.join(targetLocation, ".git"), { recursive: true, force: true })
			.catch(() => {})
		await fs
			.rm(path.join(targetLocation, "package-lock.json"), { force: true })
			.catch(() => {})
		await fs
			.rm(path.join(targetLocation, "node_modules"), {
				recursive: true,
				force: true,
			})
			.catch(() => {})
